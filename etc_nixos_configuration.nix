...

{
  imports =
    [ # Include the results of the hardware scan.
      ...
      <home-manager/nixos> # 1. Подключаем модуль
    ];
...
# Включаем поддержку Flakes
  ...
  networking.bridges.br0.interfaces = []; # Мост для связи между ВМ
...
services.ollama.package = pkgs.ollama-cuda;
  ...
  environment.systemPackages = with pkgs; [
        ...
        rustc
        cargo
        rust-analyzer # LSP для Helix
        clippy        # линтер
        rustfmt       # форматирование кода
        pkg-config    # нужен многим крейтам для поиска системных либ
        openssl       # часто требуется для сетевых библиотек Rus
        gcc
        gnumake
        cmake        # Теперь ошибка "is cmake not installed?" исчезнет
        rdma-core    # Библиотеки ibverbs
        libclang     # Нужно для bindgen, который генерирует привязки к C-коду
        ninja        # Опционально, но cmake часто его хочет
        python3      # Добавьте это
        libnl.dev       # Библиотека libnl-3.0 и libnl-route-3.0
        libclang.dev # Нужно для генерации привязок (bindgen)
  ];

  # Переменные сессии (рекомендуется для GUI и терминалов)
...
#   };
  ...
  programs.nix-ld.libraries = with pkgs; [
    # Список необходимых библиотек (обычно stdenv.cc.cc, zlib и др.)
    stdenv.cc.cc
    openssl
    curl
    zlib
...
#  };
  ...
  # 2. Настройки для вашего конкретного пользователя
     ...
     # (Опционально) Использовать системные пакеты внутри HM
